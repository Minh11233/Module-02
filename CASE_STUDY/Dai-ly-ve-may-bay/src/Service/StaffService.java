   ...
   private String RegisteredPassword;
    ...
    private void reviewRevenueAndProfitByYear() throws Exception {
        ...
        if (inputYear < 2021) {
            ...
            System.out.println("Hợp đồng phần mềm hết hạn năm 2025!!! Vui lòng gia hạn");
            reviewRevenueAndProfitByYear();
    ...
    private void checkRevenueAndProfitByMonth() throws Exception {
        ...
        if (isMonth) {
            boolean isRevenueAvalable = Last_Month_With_Revenue >= inputMonth;
            if (isRevenueAvalable) {
                for (int i = 0; i < revenues.size(); i++) {
    ...
    public void addFlightTime() throws Exception {
        ...
        if (Validate.validate(newFlightHour, "FLIGHT_TIME")) {
            if (!FlightService.flightHourList.contains(newFlightHour)) {
                ...
                System.out.println("Giờ bay đã có trong danh sách. Vui lòng thử lại");
                addFlightTime();
            }
        } else {
